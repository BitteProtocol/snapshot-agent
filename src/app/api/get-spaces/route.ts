...
type Space = { id: string, name: string, about: string, network: string, symbol: string, strategies: string[], admins: string[], members: string[], filters: string[], plugins: string[] }
...
async function fetchSpacesWithGraphQLRequest(evmAddress: string) {
  ...
  try {
    ...
    const data = await client.request<{ proposals: Proposal[] }>(GET_SPACES_QUERY);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log('\n=== PROPOSAL RESULTS ===');
    spaces.forEach((proposal: Proposal, index: number) => {
